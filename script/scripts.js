
	function Game(){
		this.clientw=document.documentElement.clientWidth;
		this.clienth=document.documentElement.clientHeight;
		this.letterArr=["A","B","C","D","E","F","G","H","I","G","K","M","L","N",];
		this.letterLen=5;
		this.speed=5;
		this.spans=[];
        this.currArr=[];
        this.currPosArr=[];
		this.play();
		this.die=10;
		this.sore=0;
        this.currSore=0;
        this.num=10;
		this.soreEle=documentElement.getElementsByClassName('sore')[0].getElementsByTagName('sore')[1];
		this.soreEle=documentElement.getElementsByClassName('die')[0].getElementsByTagName('die')[1];
		this.aa=1;
	}
	Game.prototype={
		play:function(){
			//将字母显示到body里面
			this.getLetter(this.letterLen);
			this.move();
			this.key();
		},
		key:function(){
			var that=this;
            docuemnt.onkeydown=function(e){
            	var ev=e||window.event;
            	var code=String.fromCharCode(ev.keyCode);
            	for(var i=0;i<that.spans.length;i++){
            		if(that.spans[i].innerHTML==code){
                    document.body.removeChild(that.spans[i]);
            		that.spans.splice(i,1);
                    that.currArr.splice(i,1);
                    that.currPosArr.splice(i,1);
                    that.getLetter(1)  
                    that.sore++;
                    that.currSore++;
                    that.soreEle.innerHTML=that.sore;
                    if(that.currSore%that.num==0){
                    	that.aa++;
                    	alert("")
                    	that.next();
                      }
                      break;
            		}
            	}
            }
		},
		next:function(){
			clearInterval(that.t)
			for(var i=0;i<this.spans.length;i++){
				document.body.removeChild(this.spans[i]);
			}
			this.spans=[];
			this.speed++;
            this.currPosArr=[];
            this.currArr=[];
			this.letterLen++;
			this.play()
		},
		move:function(){
			var that=this;
            this.t=setInterval(function(){
            for(var i=0;i<that.spans.length;i++){
            	var top=that.spans[i].offsetTop+this.speed
            	that.spans[i].style.top=top+'px';
            	if(top>this.clienth){
            		document.body.removeChild(that.spans[i]);
            		that.spans.splice(i,1);
                    that.currArr.splice(i,1);
                    that.currPosArr.splice(i,1);
                    that.getLetter(1)
                    that.die--;
                    that.dieEle.innerHTML=that.die;
                    if(that.die==0){
                    	alert("sile")
                    	that.restart();
                    }
            	}
            }
            },60)
		},
        restart:function(){
             clearInterval(that.t)
            for(var i=0;i<this.spans.length;i++){
                document.body.removeChild(this.spans[i]);
            }
            this.spans=[];
            this.speed++;
            this.currPosArr=[];
            this.currArr=[];
            this.letterLen++;
            this.play()
        },
		getLetter:function(num){
            //先获取到指定的字母
           var arr= this.getRand(num);
           var posArr=[];
           var eleArr[];
           for(var i=0;i<arr.length;i++){
           	var span=document.createaealement('span');
           	span.innerHTML=arr[i];
           	var x=(100+(this.clientw-200)*Math.random());
           	var y=(100+(this.clienth-100)*Math.random());
           	 var width=30;
             while(this.check1(this.currPosArr,x,width)){
             	x=(100+(this.clientw-200)*Math.random());
             }
             posArr.push({min:x,maxx:x+width});
             this.currPosArr.push({minx:x,maxx:x+width})
            span.style.cssText='width:"+width+";position:absolute;left:"+x+"px;top:"+y+";color:white;font-size:30px;'
            document.body.appendChild(span);
            this.spans.push(span);

           }
           return span;
		},
		check1:function(arr,x,width){
             for(var i=0;i<arr.length;i++){
             	if(x+width<arr[i].minx||arr[i].maxx+width<x){
             		return true;
             	}
             }
		},
		getRand:function(num){
			var arr=[];
			for(var i=0;i<num;i++){
			var rand=Math.floor(this.letterArr.length*Math.random());
			while(this.check(this.currArr,this.letterArr[rand])){
				rand=Math.floor(this.letterArr.length*Math.random());
			    }
			    arr.push(this.letterArr[rand]);
                this.currArr.push(this.letterArr[rand]);
			}
			return arr; 
		},
		check:function(arr,val){
			//检测重复
             for(var i=0;i<arr.length;i++){
             	if(arr[i]==val){
             		return true;
             	}
             }
             return false;
		}
	}























